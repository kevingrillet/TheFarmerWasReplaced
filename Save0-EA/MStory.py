...
def run():
    ...
    while True:
        ...
        if num_items(Items.Wood) < curr:
            IHayWood.farmWood(curr, True)
        if num_items(Items.Carrot) < curr:
            ICarrots.farmCarrot(curr)
        if num_items(Items.Pumpkin) < curr:
            IPumpkin.farmPumpkin(curr, -1)
        if num_items(Items.Cactus) < curr:
            ICactus.farmCactus(curr)
        if num_items(Items.Power) < 10000:
            IPower.farmPower(10000)
...
def runV2(customItems):
    ...
    if customItems == None:
        customItems = [Items.Hay, Items.Wood, Items.Carrot,
        Items.Pumpkin, Items.Cactus, Items.Bone, Items.Gold]
    ...
    while True:
        for item in customItems:
            if num_items(item) < next:
                UManager.farm(item, next, hasT, hasP)
        UManager.farm(Items.Power, 10000, hasT, hasP)
...
def runV3():
    ...
    for lvl in lvls:
        UManager.farm(Items.Power, 10000, hasT, hasP)
        UManager.getSkill(t, hasT, hasP, lvls[lvl], lvl)
